...
class publicVC: UIViewController,UITableViewDelegate,UITableViewDataSource,UIPopoverPresentationControllerDelegate{
    
    let blurView = UIVisualEffectView(effect: UIBlurEffect(style: UIBlurEffectStyle.light))
    var  CONTACTS = [Contact]()
   
    var MinistryArray = [Contact]()
    var NGOArray = [Contact]()
...
//        
        ...
        for json in jsons{
            ...
            let cont = Contact(id: id, khName: khName, EnName: EnName, phone: phone, email: email, web: web, locationLL: locationLL, type: type, category: category, description: description, status: status)
             if category == "Ministry" {
                MinistryArray.append(cont)
            }
            else  {
                NGOArray.append(cont)
            }
          //  contacts.append(cont)
        }
        self.CONTACTS = MinistryArray
        print(contacts)
    ...
    @IBAction func SwitchSeg(_ sender: UISegmentedControl) {
        if sender.selectedSegmentIndex == 0{
            ...
            self.CONTACTS = NGOArray
            self.table.reloadData()
        }
    }
    
    @IBAction func OpenConact(_ sender: Any) {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        request(url: "http://localhost:8888/beebook/public/read/Public")
        table.delegate = self
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "publicToDetail", sender: CONTACTS[indexPath.row])
...
//            viewdetailVC.view.alpha = 0
    ...
    func setDataToRow(cell:mainCell,contact:Contact) {
        ...
        if contact.category == MINISTRY{
            cell.rowIcon.image = #imageLiteral(resourceName: "goverment")
        }
        else {
            cell.rowIcon.image = #imageLiteral(resourceName: "N")
        }
